//
// This file was automatically generated. Please don't edit by hand.
//

#ifndef LIGHTINGDEBUG_CS_HLSL
#define LIGHTINGDEBUG_CS_HLSL
//
// UnityEngine.Experimental.Rendering.HDPipeline.DebugLightingMode:  static fields
//
#define DEBUGLIGHTINGMODE_NONE (0)
#define DEBUGLIGHTINGMODE_DIFFUSE_LIGHTING (1)
#define DEBUGLIGHTINGMODE_SPECULAR_LIGHTING (2)
#define DEBUGLIGHTINGMODE_LUX_METER (3)
#define DEBUGLIGHTINGMODE_VISUALIZE_CASCADE (4)
#define DEBUGLIGHTINGMODE_INDIRECT_DIFFUSE_OCCLUSION (5)
#define DEBUGLIGHTINGMODE_INDIRECT_SPECULAR_OCCLUSION (6)
#define DEBUGLIGHTINGMODE_SCREEN_SPACE_TRACING_REFRACTION (7)
#define DEBUGLIGHTINGMODE_SCREEN_SPACE_TRACING_REFLECTION (8)

//
// UnityEngine.Experimental.Rendering.HDPipeline.DebugScreenSpaceTracing:  static fields
//
#define DEBUGSCREENSPACETRACING_NONE (0)
#define DEBUGSCREENSPACETRACING_COLOR (1)
#define DEBUGSCREENSPACETRACING_HIT_DEPTH (2)
#define DEBUGSCREENSPACETRACING_HIT_SUCCESS (3)
#define DEBUGSCREENSPACETRACING_TRACING_MODEL (4)
#define DEBUGSCREENSPACETRACING_SPECULAR_COLOR (5)
#define DEBUGSCREENSPACETRACING_HI_ZITERATION_COUNT (6)
#define DEBUGSCREENSPACETRACING_HI_ZINTERSECTION_KIND (7)
#define DEBUGSCREENSPACETRACING_HI_ZHIT_WEIGHT (8)
#define DEBUGSCREENSPACETRACING_LINEAR_ITERATION_COUNT (9)
#define DEBUGSCREENSPACETRACING_LINEAR_HIT_WEIGHT (10)


#endif
